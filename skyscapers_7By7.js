...
function solvePuzzle(clues, n=N) {
	...
	dual_clues = dual_clues.sort((a,b)=>Math.max(clues_with_priority[b[0]].value,clues_with_priority[b[1]].value) - Math.max(clues_with_priority[a[0]].value, clues_with_priority[a[1]].value)).reduce((a,e)=>a.concat(e.sort((i,j)=>clues_with_priority[j].value - clues_with_priority[i].value)),[]);
	...
	clues_with_priority = sorted_clue_with_priority;
...
function test() {
	...
	return solvePuzzle(clues_7_2,7);
}
